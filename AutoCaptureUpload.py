...
def capture_screenshots(video_path, num_screenshots=3):
    # 生成随机的截图时间点
    video_duration = get_video_duration(video_path)
    screenshot_times = random.sample(range(1, video_duration), num_screenshots)

    # 创建一个文件夹存储截图
    screenshots_dir = os.path.dirname(video_path) + "/screenshots"
    os.makedirs(screenshots_dir, exist_ok=True)

    # 使用ffmpeg截取视频截图
    for i, timepoint in enumerate(screenshot_times):
        output_file = f'{screenshots_dir}/screenshot_{i+1}.jpg'
...
with open("config.yml", "r") as file:
    config = yaml.safe_load(file)
capture_screenshots(video_path,config.get("num_screenshots"))
